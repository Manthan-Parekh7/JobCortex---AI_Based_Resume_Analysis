...
def analyze_full_resume(
    ...
    ai_issues_raw = parsed_resume.get('issues_and_fixes', [])
    ai_issues = []
    if isinstance(ai_issues_raw, list):
        for item in ai_issues_raw:
            if not isinstance(item, dict):
                continue
            try:
                ai_issues.append(IssueDetail(**item))
            except Exception:
                continue

    if ai_issues:
        detailed_feedback = ai_issues
        issues_summary = generate_issues_summary(ai_issues)
    else:
    ...
    summary_text = (parsed_resume.get('professional_summary', '') or '').strip()
    ai_summary = (parsed_resume.get('ai_summary', '') or '').strip()
    summary_output = summary_text or ai_summary
